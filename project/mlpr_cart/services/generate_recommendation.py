...
# # Load model & encoder
# model = joblib.load("project/mlpr_cart/models/model.joblib")
# encoder = joblib.load("project/mlpr_cart/models/encoder.joblib")
# product_meta = pd.read_parquet("project/mlpr_cart/data/product_metadata.parquet")
# category_pairs = pd.read_csv("project/mlpr_cart/data/category_pairs.csv")
...
# Langsung tentukan BASE_DIR sebagai 'project/mlpr_cart'
BASE_DIR = Path(__file__).resolve().parents[1]
print(f"📂 BASE_DIR resolved to: {BASE_DIR}")  # /project/mlpr_cart  # Adjust as needed to point to your project root
...
def load_assets():
    try:
        ...
        encoder = joblib.load(BASE_DIR / "models" / "encoder.joblib")
        product_meta = pd.read_parquet(BASE_DIR / "data" / "product_metadata.parquet")
        category_pairs = pd.read_csv(BASE_DIR / "data" / "category_pairs.csv")
        category_pairs["category_paired"] = category_pairs["category_paired"].apply(safe_parse_category_list)
        return model, encoder, product_meta, category_pairs
...
def get_candidate_recommendations(input_product, product_meta, category_pairs, model, encoder):
    input_row = product_meta[product_meta["product_id"] == input_product]
    if input_row.empty:
        return None, None
...
def run_batch():
    ...
    model, encoder, product_meta, category_pairs = load_assets()
    product_ids = fetch_recent_cart_products()
    for pid in product_ids:
        input_name, recs = get_candidate_recommendations(pid, product_meta, category_pairs, model, encoder)
        if recs:
            update_recommendation_db(pid, recs)
            print(f"✅ Updated for {pid} ({input_name}) → {len(recs)} recommendations")
        else:
